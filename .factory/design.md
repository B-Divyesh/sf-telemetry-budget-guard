...
- `site/public/night-market-telemetry.webp` and its responsive `-720` derivative: original AI-generated editorial illustration, used as explanatory atmosphere behind the estimator signboard. Generated 2026-08-27 with the factory `factory-image` deployment via `/opt/fleet/lib/gen-image.sh`, then converted locally to WebP (123 KB desktop, 43 KB mobile). Prompt: “Wide editorial night-market scene reimagined as an observability data bazaar; small stalls made from dark server racks; glowing cyan trace ribbons, amber metric tokens, and coral log receipts flow toward a single illuminated budget gate; screen-print and cut-paper texture; deep ink-black background; cinematic but restrained; strong negative space on the left for landing-page copy; no people, no readable text, no logos, no gradients, no watermark; 3:2 landing-page hero.” Generated asset is original for this product; project use under the repository MIT license.
- `site/public/og-image.webp`: 1200×630 crop of the original night-market illustration, composed locally for link previews. It adds no third-party material.
- `site/public/apple-touch-icon.png`: 180×180 raster rendering of the hand-authored project favicon, composed locally for device bookmarks.
